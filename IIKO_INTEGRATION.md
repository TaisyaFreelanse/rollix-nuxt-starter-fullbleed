...
### Настройка прав доступа в iiko Web

**ВАЖНО:** Для работы с iikoCloud API необходимо настроить права доступа в iiko Web.

1. Откройте **iiko Web** → **Настройки** → **Права доступа**
2. Найдите ваш API ключ (или создайте новый)
3. Убедитесь, что для API ключа включены следующие права:
   - ✅ **Работа с API** - необходимо для всех запросов к API
   - ✅ **Просмотр внешнего меню** - необходимо для получения номенклатуры и меню
   - ✅ **Создание заказов** - необходимо для отправки заказов в iikoDelivery
   - ✅ **Просмотр заказов** - необходимо для получения статусов заказов

**Если права не настроены**, вы получите ошибку `401 Unauthorized` при попытке синхронизации меню или создания заказов.

**Как проверить права:**
- В iiko Web перейдите в раздел настроек API
- Проверьте список разрешений для вашего API ключа
- Убедитесь, что все необходимые права активны
...
3. **При ошибках авторизации (401)** токен автоматически сбрасывается, запрашивается новый токен и запрос повторяется. Если ошибка повторяется, проверьте настройки прав доступа в iiko Web.
...
6. **Ошибка 401 (Unauthorized)** обычно означает:
   - API ключ неверный или истек
   - У API ключа нет необходимых прав доступа в iiko Web
   - OrganizationId указан неправильно
   
   **Решение:** Проверьте настройки прав доступа в iiko Web (см. раздел "Настройка прав доступа" выше).
